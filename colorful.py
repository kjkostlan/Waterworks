# Why is color so rarely used!
from . import global_vars
tprint = global_vars.tprint # All Waterworks prints are thread-safe.
...
def bprint(*txt):
    ...
    tprint('\033[94m'+txt+'\033[0m', end='')

def oprint(*txt):
    # Orange.
    txt = ' '.join([str(t) for t in txt])
    tprint('\x1b[0;33;40m'+txt+'\x1b[0m')

def arrowprint(*txt):
    txt = ' '.join([str(t) for t in txt])
    tprint('\x1b[0;33;40m'+'→'+'\x1b[6;30;42m'+txt+'\x1b[0;33;40m'+'←'+'\x1b[0m')

def arrowprint1(*txt):
    # Slightly different.
    txt = ' '.join([str(t) for t in txt])
    tprint('\x1b[0;33;40m'+'→'+'\033[97;104m'+txt+'\x1b[0;33;40m'+'←'+'\x1b[0m')
